...
class grid():
    ...
    def __init__(self, dataConfig, topo, start_date, end_date,
        ...
        if dataType == 'wrf':
            ...
            self.load_from_hrrr()
        elif dataType == 'hrrr_netcdf':
            self.load_from_hrrr_netcdf()
        else:
...
#             setattr(self, v, d.tz_localize(tz=self.time_zone))
    ...
    def load_from_hrrr_netcdf(self):
        """
        Load the data from the High Resolution Rapid Refresh (HRRR) model that
        is contained in netcdf files on an opendap server.
